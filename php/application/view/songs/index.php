<div class="container">
    ...
    <div class="box">
        ...
        <form action="<?php echo URL; ?>songs/addtraining" method="POST">
            <label>Plik</label>
            <input type="file" name="xml" value="" required />
            <!--<label>Tytuł</label>
            <input type="text" name="title" value="" required />
            <label>Typ</label>
            <input type="text" name="type" value="" />-->
            <label>Distance</label>
            <input type="number" name="distance" value="" required />
            <label>Time</label>
            <input type="time" name="t_time" value="" required />
            <label>Calories</label>
            <input type="number" name="calories" value="" />
            <!--<label>Opis</label>
            <input type="text" name="description" value="" required />
            <label>Data</label>
            <input type="text" name="created" value="" required /> -->
            <input type="submit" name="submit_add_training" value="Submit" />
        </form>
    </div>
    <!-- main content output -->
    <div class="box">
        ...
        <table>
            ...
            <?php foreach ($trainings as $training) { ?>
                <tr>
                    <td><?php if (isset($training->id)) echo htmlspecialchars($training->id, ENT_QUOTES, 'UTF-8'); ?></td>
                    <td><?php if (isset($training->distance)) echo htmlspecialchars($training->distance, ENT_QUOTES, 'UTF-8'); ?></td>
                    <td><?php if (isset($training->t_time)) echo htmlspecialchars($training->t_time, ENT_QUOTES, 'UTF-8'); ?></td>
                    <td><?php if (isset($training->calories)) echo htmlspecialchars($training->calories, ENT_QUOTES, 'UTF-8'); ?></td>
                    <td><a href="<?php echo URL . 'songs/deletetraining/' . htmlspecialchars($training->id, ENT_QUOTES, 'UTF-8'); ?>">delete</a></td>
                    <td><a href="<?php echo URL . 'songs/edittraining/' . htmlspecialchars($training->id, ENT_QUOTES, 'UTF-8'); ?>">edit</a></td>
                </tr>
            <?php } ?>
            </tbody>
        </table>
